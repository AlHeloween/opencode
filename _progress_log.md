...
CONFIRMED (✓ счётчики частей, ✓ transform.ts:284 + live raw-wire `18-32-34…json`): пост-фолд ход 09:15:04Z — 0/0/0; ход после вопроса о марках (09:16:16Z) — 172/120/58; reasoning уходит эхом в следующие запросы (в свежем raw-wire виден собственный reasoning внутри тела) ⇒ форма марок в мышлении приходит и уходит с ТЕМОЙ в окне, а не держится правилом (модель сама это сформулировала хвостом B: «твой вопрос вернул тему (и форму) в окно… следующий фолд снова их снесёт. Это предсказание»).

Открыто (в артефакте): 11 частей без строк `message` в окне 09:15:01–09:18:18Z (материализованный m* на 253 284 байта + пост-фолд ход + вопрос + B) — механизм Unknown; coupling-счётчик по окну (фолд честно даёт 17→0, но изнутри это петля); границы марок в thinking не решены.
...
## [2026-09-26 04:16Z] cua-driver: 0.24.0 → 0.29.1 via its own updater; two install surfaces reconciled

CONFIRMED (✓ `where cua-driver` → `C:\Users\Alexander\AppData\Local\Programs\Cua\cua-driver\bin\cua-driver.exe`; ✓ `check-update --json --no-cache`: current 0.24.0, stable 0.29.1, `update_available: true`; ✓ `update --apply` exit 0 — installed `0.29.1-x86_64-pc-windows-msvc` under `~/.cua-driver/packages/releases/`, with `AppData\...\bin` a junction to `packages/current`; ✓ daemon from the updated install listening on `\\.\pipe\cua-driver` (cmd_runner run `20260926T041158Z_bd725352`); ✓ `health_report` overall `ok` (UIA + D3D11 capture pass); ✓ `check_for_update`: current = latest = 0.29.1, `update_available: false`; ✓ `bin/cua` re-aligned to the same release — SHA-256 `c0dc4bdf8d2b24e785769c82fb77fc618b9f3b2263e113d565167e4e38acda57` equals the release artifact, `bin\cua.cmd --help` answers 0.29.1; old binary kept at `experiments/2026-09-26_cua-driver-update/cua-driver.exe.prev` (`d7a90c2a082bf650cf9caea51cc18ee88c3689fb2b5f3897bb93697a135cf81e`).

Mechanism of the incident: two install surfaces existed — the canonical Cua installer copy (on PATH, preferred by `tool/cua.ts:21-24` via `which()`) and the vendored `bin/cua` self-build (2026-09-08) — and an interim source build from `external/cua` (contract 0.8.0) clashed with the installed 0.24.0 client (SDK 0.7.0): `incompatible daemon: contract version 0.8.0 does not match SDK 0.7.0`. Resolution: ONE version on both surfaces (0.29.1, stable channel); the `external/cua` clone stays as reference only. `docs/tools-and-sidecars.md` §7.1 updated with the update path and the PATH-priority rule.

Residual: WebView2 click precision (owner's field report — synthetic clicks landing off-target, 1–2 px marker hits unreliable) is not measurable from this session without the target window; candidate fixes ride 0.29.1 (#4188 Chromium-frame background-click refusal, #4196 foreground escalation, #4192 capture-bound click E2E), and the final visual acceptance for 3D/print flows is assigned to the agent's own eyes rather than synthetic clicks.

## [2026-09-26 06:53Z] mermaid: height flows, width clamps — the 09-23 row-budget re-fit removed

CONFIRMED (✓ `bun test test/util/mermaid.test.ts test/util/fit-image.test.ts`: 42 pass / 0 fail / 88 expect, cmd_runner run `20260926T065210Z_ea1fd075`, exit 0): `resvgOptionsForSvg` no longer re-fits by height — `maxHeight` is accepted for compatibility and has NO effect; only the width clamp inside `fitFontAnchoredSize` may reduce the scale. Owner ruling, verbatim: «клампить ширину, высоту отпускать и вставлять как есть» (2026-09-26), because a diagram may legitimately be 2000–3000 px tall. New test pins it: a tall diagram keeps `fitTo.mode = "width"` even when the passed row budget is tiny. Docs `rendering.md` §6e/§6f corrected: height re-fit marked as the regression it was; "registerMermaidFont never called" and "media-mermaid dead code" rows closed against the current tree (`ensureMermaidFont` runs on every render; `routes/session/index.tsx:69,2295` renders `<MediaMermaid>`). Residual: live `bin/opencode.exe` predates the fix (needs a candidate build to re-verify on screen); the PNG fallback path still renders without a budget (`media-mermaid.tsx:23`) — a separate candidate.

## [2026-09-26 07:55Z] T1 unified-settings-layers: global seed — free zen + vision + max context, big-pickle fail-protection only

CONFIRMED (✓ `bun test test/provider/free-default.test.ts test/session/fill-layers.test.ts`: 23 pass / 0 fail / 59 expect, run `20260926T075148Z_86a80007`, exit 0; ✓ `bun typecheck`: exit 0, run `20260926T075202Z_7bbf0c56`): new `pickFreeVisionModel` (`src/provider/free-default.ts`) ranks zen (`opencode`) models by `cost 0/0` + `capabilities.input.image` + `toolcall !== false`, largest `limit.context` first; `big-pickle` is excluded from the preferred pick and returned only when nothing else qualifies — owner, 2026-09-26: «Big-Pickle это fail protection и не рекомендуется». `seedGlobalLayer` (`local.tsx:656-693`) seeds the GLOBAL layer with ONE config PATCH when no agent declaration holds a model; toast marks the fail-protection case. The `fillSourceFor` tail (`local.tsx:501-505`) now takes the recommended seed before the arbitrary tool-calling fallback, so worktree/session fills on an empty global copy the same pick. Spec and remaining work: `plans/2026-09-26_unified-settings-layers.md` (T2 auto-tab by phase, T3 worktree-pick→session, T4 server-fallback audit, T5 smoke + candidate build). Residual: if zen publishes only big-pickle as free, the fail-protection branch is what live configs will show — verify against the live catalog on the candidate build.

## [2026-09-26 09:20Z] T2 unified-settings-layers: /agents opens on the PHASE layer (global → worktree → session)

CONFIRMED (✓ `bun test test/tui/agent-selection.test.ts`: 9 pass / 0 fail / 25 expect, run `20260926T091852Z_1499c10e`, exit 0; ✓ `bun typecheck`: exit 0, run `20260926T091902Z_fcffee53`): new pure `phaseScope` (`config-scope.ts`) maps the fill phase to the layer a settings dialog opens on — `global` while the worktree layer is not materialised, `worktree` while the session layer is empty (new session), `session` once every layer holds values. `local.tsx` exposes it as a model method (globalFilled = any agent declaration; worktreeFilled = current workspace scope non-empty; sessionFilled = active session's agent map non-empty); `dialog-agent.tsx:48` and `dialog-model.tsx` now default through it, and `dialog-model.tsx` lost its kv `readScope`/`stored` fallback (the 2026-09-21 stale-kv bug this replaces). A manual scope switch in an open dialog still wins via `props.scope`. Spec/next: `plans/2026-09-26_unified-settings-layers.md` T3 (worktree-pick → session in the new-session phase), T4 (server fallback audit), T5 (smoke + candidate build).

## [2026-09-26 09:27Z] T4 (step 1): the server records a session-layer fall-through instead of guessing

CONFIRMED (✓ `bun typecheck`: exit 0, run `20260926T092654Z_3083dabf`): `createUserMessage` (`prompt.ts:984-997`) resolves `fromSession = input.model ?? sessionWorkspaceModel` first and emits `bug: prompt model fell through the session layer` (agent + the fallback used: `agent-declaration` | `last-message/default`) when a prompt has to leave the session layer; wire behaviour unchanged. Owner policy, 2026-09-26: «observability на берегу — мы знаем с какими настройками лезем» — the configuration is known BEFORE the request and the warn is a fill-defect signal, not a working branch. Addendum recorded in `plans/2026-09-26_unified-settings-layers.md` §5: no-session phase semantics, protocol/routing as settings (T6), `auto(resolved)` display, protocol fixed at settings time, observability-on-the-shore.

## [2026-09-26 09:29Z] Candidate build 10.0.1119 — T1 + T2 + T4-warn + mermaid height-flow

CONFIRMED (✓ `python build.py --only opencode`: exit 0, cmd_runner run `20260926T092750Z_a904dbf0`, 43.4s; ✓ embedded smoke `dist/opencode-windows-x64/bin/opencode.exe --version` → `10.0.1119`; ✓ `reasoning_prompt.txt` embedded): candidate at `packages/opencode/dist/opencode-windows-x64/bin/opencode.exe` carries T1 (global zen seed, big-pickle fail-protection), T2 (phase tabs global → worktree → session), T4 step 1 (session-layer fall-through warn in `createUserMessage`) and the mermaid fix (width clamps, height flows). Live check to be run from `dist/`, not `bin/` (owner views it himself).

## [2026-09-26 09:37Z] T6 (part 1): sidebar protocol cell — auto(<resolved>) / <protocol>

CONFIRMED (✓ `bun test test/tui/sidebar-protocol.test.ts`: 6 pass / 0 fail / 17 expect, run `20260926T093646Z_8240877c`, exit 0; ✓ `bun typecheck`: exit 0, run `20260926T093658Z_39ad61a0`): `protocolLabel` (`protocol-row.ts`) renders the CONFIGURED mode with `auto` wrapping the measured transport — `auto(h2)` when the layer says auto (or says nothing), the protocol itself when fixed; never empty (AGENTS.md: absence reads as FALSE). The sidebar reads the configured value from the model's own layer (`model.options.protocol ?? "auto"`, `context.tsx`) and dropped the unconditional `h2 (protocol)` cell. Owner cite, 2026-09-26: «сейчас в tui: h2 (protocol) — если auto → auto(выбранный протокол), если не auto протокол». Residual (T6 part 2): per-session protocol override still reports «needs server work» (`setModelProtocol` session scope: SDK options are cached, a per-session override needs the gateway to take it at request assembly), and the resolve-at-settings-time rule; candidate 10.0.1119 does not carry part 1.

## [2026-09-26 11:47Z] Neighbour-session capture removed: scopes and phase follow the OPEN session (route), not the newest one

CONFIRMED (✓ `bun test test/tui/agent-selection.test.ts`: 9 pass / 0 fail / 25 expect, run `20260926T114729Z_8166611b`; ✓ `bun typecheck`: exit 0, run `20260926T114708Z_0448f2af`; instrument note: the first typecheck of this change died `exit_code=-1` after 15 s with 304 bytes and no output — a process flake, re-run green): `phaseScope` counts a session as open only when `route.data.type === "session"` — `home` resolves the newest session for the settings surfaces, and that was exactly the neighbour whose session values `/agents` offered as editable (owner, 2026-09-26: «подхватывает соседнюю сессию, параметры которой нельзя менять, а если поменять — то вообще что поменяется. Конфликт»). Route-based scopes now also in `dialog-agent.tsx`, `dialog-model.tsx`, `dialog-routing.tsx`, `dialog-variant.tsx`: the session tab exists only with an open session. `dialog-agent` hint carries the missing note — «No session yet — edits land in the <layer> layer; the next session is filled from it». `activeSessionID` no longer appears in any dialog (grep: definition + `getActiveSessionID` only). Candidate needs a REBUILD to include this fix.

## [2026-09-26 13:23Z] T3: a worktree pick reaches the OPEN session for ANY agent (DeepSeek in worktree was invisible to the runtime)

CONFIRMED (✓ `bun test test/tui/agent-selection.test.ts`: 9 pass / 0 fail / 26 expect, run `20260926T132314Z_e5fd86b7`; ✓ `bun typecheck`: exit 0, run `20260926T132328Z_0cfadd0a`): `shouldUpdateSessionModelOnPick` is now `hasSession && scope !== "global"` — the `targetAgent === activeAgent` guard is gone, so the worktree tab no longer leaves the open session on its old model (owner, 2026-09-26: «я выбрал deepseek в worktree — а стоит бигпикл», «Как было дерьмо так и осталось»). The write still never moves the ACTIVE agent; it now also refuses to write without an OPEN session (`route.data.type === "session"` in `local.model.set`), so the captured-neighbour defect cannot return through this path. Candidate rebuilt: `10.0.1122`, exit 0, embedded smoke `dist/opencode-windows-x64/bin/opencode.exe --version` → `10.0.1122` (run `20260926T132349Z_5c804dea`).

## [2026-09-26 13:28Z] Live candidate smoke (10.0.1122): seed, phase, note — verified in a real TUI

CONFIRMED (✓ cmd_runner run `20260926T132726Z_69d0ba92`, isolated worktree `experiments/2026-09-26_candidate-smoke`, render read from the session log): the screen shows «Global config seeded — opencode/muse-spark-1.3-contributor-free — 10 agents» and the status line «Build · Muse Spark 1.3 Free OpenCode Zen [🎥 👁 🧠 🔧]» — T1 seeds free+vision+1M, NOT big-pickle. `/agents` (sent `TEXT:/agents,ENTER` over the cmd_runner inbox) opens «Agent Configuration — scope: worktree», with NO session tab, and the hint «No session yet — edits land in the worktree layer; the next session is filled from it · … · model from the worktree layer» — T2/T2b verified live. Instrument note: the candidate first died `exit_code=1` with `Failed to open library "B:\~BUN\root\opentui.dll": error code 126` because `build.py --only opencode` does NOT stage `opentui.dll`; it lives at `packages/opentui/packages/core-win32-x64/opentui.dll` and `build.py:247-248` copies it during the `stage` step — a candidate needs it staged next to the exe before any live run.

## [2026-09-26 13:33Z] HALF-WRITE found: a model pick updated recent/variant/session but NOT the worktree layer (Big Pickle stayed)

EVIDENCE (files of this worktree, mtime 21:32:10 local — same second): `.opencode/data/state/model.json` holds `recent=[deepseek/deepseek-flash]`, `variant`/`agentVariant` = `deepseek-flash: max`, while `workspaceAgent.default.build_mode` is still `opencode/big-pickle`; `sessions/ses_f2440584affe3fvjGHrUFqtoqX.jsonc` was written the same second (session side). Owner's screenshot matches: prompt «Build · DeepSeek V4.1 Flash … · max» vs `/agents` worktree row «Big Pickle». So the pick reached recent/variant/(S2) and did NOT reach S1. CANDIDATE CLASS: every `local.model.set` path where `options.agent` is absent — S1/S2 are written only inside `if (options?.agent)` while `recent`/`variant`/`save()` still run → a half-applied write with no warning (`app.tsx:410` startup `--model` passes only `{recent:true}`; `prompt/index.tsx:252` restore passes nothing; `performSelect` can resolve an empty `agent.current()`). NEXT (after fold): reproduce one path, make the S1/S2 write impossible to skip silently, add the regression test to `test/tui/agent-selection.test.ts`.

## [2026-09-26 13:48Z] Live pick test (candidate 10.0.1123): 3 picks on the start screen OK; a SESSION-scope pick does NOT stick in the session layer (2/2)

CONTEXT: owner request — «выбери модель 3 раза и убедись что она поменялась на стартовой картинке». Live run `20260926T133631Z_2fd7e029` (cwd `experiments/2026-09-26_candidate-smoke`, version `10.0.1123`, exe built 21:30 from a tree last touched 21:23 — build is current).

CONFIRMED (✓ renders + files): three picks via `/agents` → Enter → picker each landed on the HOME start screen: Muse Spark 1.3 Free → SpaceXAI: Grok Build 0.1 → Anthropic: Claude Haiku Latest → Big Pickle; after each, `state/model.json` `workspaceAgent.default.build_mode` + `recent` held the picked value. The `/agents` worktree tab row moved with every pick.

DEFECT (✓ reproduced 2/2, live): with a session OPEN, a pick in the SESSION tab (`/agents`, scope: session, build_mode) does NOT stick in the session layer. Picked Muse Spark 1.3 Free, then Nemotron 3 Ultra Free: each time `sessions/ses_f220d4ab1ffeiyQeJp6S0ySVQV.jsonc` was REWRITTEN at pick time (mtime 21:42:13, then 21:46:08) yet still held `build_mode: opencode/big-pickle` and the PRE-pick `recent`; the `/agents` session row and the prompt (`Build · Big Pickle OpenCode Zen`) kept the old model while the worktree tab and `model.json` moved to the new one — the same class as the owner's screenshot, mirrored.

MECHANISM (Inferred): the pick's own S1 write re-triggers the fill effect (`local.tsx:413-419` reads `modelStore.workspaceAgent` reactively) → `refreshSessionSettings()` (`:217-249`) loads the session file while the pick's `saveSessionSettings` is still queued (`session-settings.ts:475` chains `pendingWrites`; `loadSessionSettings` does not wait for it) → `setSessionSettings(disk)` + `setModelStore("recent", disk)` clobber the fresh state → the later variant-step write (`saveSessionSettings(sid, sessionPayload())`) persists the clobbered state over the pick's correct write.

FIX (next bounded task): order the session-file READ after local writes (`loadSessionSettings` awaits the path's pending write, or the refresh awaits it), regression test in `test/tui/agent-selection.test.ts`, rebuild the candidate, re-run this exact live test (session-tab pick → prompt must flip).

TESTING NOTE: `TEXT:<query>,ENTER` in ONE cmd_runner batch can select a NON-top row (observed: «pickle» selected `openrouter/~anthropic/claude-haiku-latest` instead of Big Pickle); split text and Enter across two calls when the row matters.

## [2026-09-26 14:06Z] Two settings-layer defects fixed + live-verified on 10.0.1126 — home read a neighbour session; a session-scope pick was clobbered by its own refresh

DEFECT A (✓ reproduced live on 10.0.1123/1124): the first screen answered with a NEIGHBOUR session's layer. `forAgent` (`local.tsx:462`) chose the read layer via `getActiveSessionID()`, which on `home` returns the NEWEST session (that resolution exists for WRITE bookkeeping) — owner's live report on 10.0.1124: prompt «Build · DeepSeek V4.1 Flash · max» while `/agents` (phase = worktree) showed «Big Pickle». This is also why «выбери модель 3 раза» failed for him: a worktree pick writes S1, but the home prompt was reading a session that no pick had touched. FIX: `readLayer(routeType)` (`tui/util/agent.ts`) — a model read targets the session layer ONLY while `route.data.type === "session"`; `home` reads the worktree (the layer a session created from here is filled from). Same predicate the dialogs already use.

DEFECT B (✓ reproduced live 2/2 on 10.0.1123): a pick in the SESSION tab did not stick — `sessions/ses_….jsonc` was REWRITTEN at pick time yet still held the OLD model, the prompt and the `/agents` session row kept it, while the worktree layer moved to the picked model. MECHANISM: the pick's own S1 write re-triggers the fill effect (`local.tsx:413-419` reads `modelStore.workspaceAgent`), which fires `refreshSessionSettings()`; `loadSessionSettings` (`session-settings.ts:367`) read the file WITHOUT waiting for the pick's queued `saveSessionSettings` (fire-and-forget, `pendingWrites` chain at `:477-486`) → the refresh clobbered the fresh signal + `recent` with the disk's pre-pick state → the variant-step write then persisted the clobbered state over the pick. FIX: `loadSessionSettings` now AWAITS the path's `pendingWrites` entry before reading (read-after-write order); a failed write is logged there and the load still tries the file.

ORACLES (all on the rebuilt candidate `10.0.1126`): ✓ `bun test test/session/session-settings-persist.test.ts test/tui/agent-selection.test.ts` — 38 pass / 0 fail / 78 expect (run `20260926T135642Z_dcb6290b`; new tests: «a load cannot overtake a queued save», «the first screen reads the WORKTREE layer»). ✓ `bun typecheck` exit 0 (run `20260926T135655Z_d6d24aff`). ✓ live run `20260926T135811Z_f7af66d8` (cwd `experiments/2026-09-26_candidate-smoke`, home WITH the neighbour session present): launch → start screen `Build · Nemotron 3 Ultra Free` (the WORKTREE value, not the neighbour's Big Pickle); worktree pick «Space Bunny Free» → start screen flipped; then a session («hi», answered) → `/agents` (scope: session) → pick `Muse Spark 1.3 Free` → prompt `Build · Muse Spark 1.3 Free`, `/agents` session row = Muse, `sessions/ses_f21fc52f5ffevHWp8IWd6cUSkv.jsonc` = `opencode/muse-spark-1.3-contributor-free` with fresh `recent`, `state/model.json` build_mode = muse (the worktree learned the session-scope pick).

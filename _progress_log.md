...
- **The runtime refused the owner's capitalization AND the underscore.** `Tool.define` requires lowercase ASCII alphanumerics (`Tool runtime ID "temp_enable" must use lowercase ASCII alphanumerics`), so the model-facing names are `tempenable` / `tempdisable`. Renamed everywhere the name is recognised — the registry's import, service, map and builtin list, plus `dsml-normalizer`, without which a DeepSeek-emitted call is not recognised at all.

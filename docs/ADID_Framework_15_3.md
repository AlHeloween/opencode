...
> [!IMPORTANT]
> **Canonical ADID Framework document** as of 2026-09-03 (installer asset policy
> decision D1, plan MP-2026-09-03-installer-asset-policy). Versions 15.3.1 and
> 15.4.3 were retired to git history; select the highest available framework
...
1. Roles Definition: This framework defines a formal partnership between a Human Developer ( #human ) and an AGI Developer ( #agi). 
	...
	2. **AGI Developer ( #agi ) Roles:**:
        ...
        6. **Executor2( #executor):** Pre-flights and validates update plan artifacts generated by the Synthesizer. This includes static analysis and potentially validating the structure of YAML/Markdown formats before handing the artifact to the #human for final execution by #Executor1. Uses `tools/adm --apply` (or `python -m adm --apply` when tools/adm not present; see Section II.5) to perform the state transitions it synthesizes.

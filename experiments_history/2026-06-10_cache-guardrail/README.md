...
The scripts are self-contained (no package imports), so they run from the repo root:

```bash
# Phase 2: Deterministic LCP detection (no API key needed, 18 tests)
bun test experiments_history/2026-06-10_cache-guardrail/phase2_divergence.test.ts

# Phase 3: Semantic equivalence (needs Python + sentence-transformers)
pip install sentence-transformers torch numpy scipy
python experiments_history/2026-06-10_cache-guardrail/phase3_semantic.py

# Phase 4: Guardrail decision matrix (smoke test, no API key)
bun run experiments_history/2026-06-10_cache-guardrail/phase4_guardrail.ts

# Phase 4 guardrail demo with divergence detection
bun run experiments_history/2026-06-10_cache-guardrail/phase2_divergence.ts
```

History: lived at `packages/opencode/test/experiments/20260610_cache_guardrail/` until
2026-09-15, then archived to `experiments_history/` so the record is tracked in git.

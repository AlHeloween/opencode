...
RUNTIME_RULES = MappingProxyType({
    ...
    "SMOKE_SPEC": "smoke_before_spec(task) generates a SMOKE.BEFORE template: {smoke_na, baseline[], post_checks[], blast_radius}. Agent fills in concrete runnable commands before Gate 4 approval. Blast radius inferred from task keywords.",
    "SMOKE_VALIDATE": "smoke_before_validate(spec) enforces the SMOKE.BEFORE contract: smoke_na requires justification, baseline must have ≥1 check with label+cmd+expected_exit, tolerance>0 requires tolerance_reason. Returns (is_valid, diagnostic). Gate 4 rejects invalid specs.",

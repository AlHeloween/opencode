...
RUNTIME_RULES = MappingProxyType({
    ...
    "FRACTAL.CANDIDATES": "generate_fractal_candidates(model, seeds, depth) dispatches fractal generation: Sierpinski (triangle subdivision for >=3 peaks), Quad/Oct (grid subdivision for 2/4/8 peaks), L-System (grammar walk, fallback for unknown models).",
    "GOAL.SEEDS": "goal_seeds(goal, evidence) extracts meaning-true goal slices: keyword extraction -> co-occurrence clustering -> seed vectors (capped at 8). Replaces manual seed selection.",
    "RESIDUAL.LOOP": "residual_recluster(state, original_goal_sv) closes the ADID loop: re-clusters pending tasks against original Goal SV using adaptive_tau at 70th percentile. Returns tasks aligned with the original goal; at least one task always survives.",
    ...
    "GOAL.PEAKS": "goal_peaks(goal, evidence) counts distinct keyword clusters (peaks) in goal+evidence. Feeds select_fractal_model: 1 peak→L-System, 2→Quad-Oct binary, 3→Sierpinski, 4→Quad-Oct quad, 5-7→Sierpinski, 8→Quad-Oct oct, 9+→Sierpinski clamped.",
    "SV.DELTA": "sv_delta(current_sv, previous_sv) computes L1 semantic distance between two SV states (keyword→weight dicts). Returns float in [0,2]: <0.3→L-System (stable), >=0.3→Quad-Oct (moderate shift), >=0.6→Sierpinski (large shift). Neutral 0.5 if SV missing.",
...
RUNTIME_RULE_OWNERS = MappingProxyType({
    ...
    "DECOMPOSE": "plan",
    "FRACTAL.CANDIDATES": "plan",
    "GOAL.SEEDS": "plan",
    "RESIDUAL.LOOP": "verification",
    "GROUND": "evidence",
    "GOAL.PEAKS": "plan",
    "SV.DELTA": "verification",

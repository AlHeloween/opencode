...
RUNTIME_RULES = MappingProxyType({
    ...
    "NAMING": "rule identifiers use UPPER_SNAKE_CASE with underscore '_' delimiter (PEP 8 convention). Dots '.' and hyphens '-' are forbidden in new rule names. Legacy dotted rules (EVIDENCE.ORDER, SEARCH.ORDER, REUSE.BEFORE, INFOMARK.SEP, WRITE.SCOPE, CACHE.STABILITY, VERIFY.OUTCOME, ADID.FREEZE, ADID.OPS, NO_HARDCODE, WHERE_WHICH, SV_OUTPUT, CLEAN_STATE, DECOMPOSE, DOCUMENT.SURFACE, WORKSPACE.LANES, PROGRESS.LOG, MEMORY.RANK, MEMORY.LINKS) are grandfathered — do not extend the dotted namespace with new entries.",
...
RUNTIME_WORKFLOWS = MappingProxyType({
    ...
    "hygiene": ("NAMING", "DOCUMENT.SURFACE", "WORKSPACE.LANES", "PROGRESS.LOG", "CLEAN_STATE", "EVIDENCE.ORDER"),
    "plan": (
        "plan",
        "DECOMPOSE",
        "SMOKE_BEFORE",

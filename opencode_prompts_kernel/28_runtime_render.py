...
def render_algorithm_card() -> str:
    ...
    lines = [
        ...
        "    ground,                      # evidence-gathering plan from goal keywords",
        "    goal_seeds,                  # keyword extraction → cluster → seed vectors",
        "    goal_peaks,                  # count distinct keyword clusters (→ select_fractal_model)",
        "    sv_delta,                    # L1 distance between two SV states (→ delta_v)",
        ...
        "    emit_state,                  # structured state record (done/pending/blocked/next)",
        ...
        "    completed = []  # tasks that passed oracle verification",
        "    pending = []    # tasks not yet executed or oracle-failed",
        "    blockers = []   # tasks blocked by external dependency",
        "    for task in central_tasks:  # one in_progress at a time (PLANNING invariant)",
        "        status, output = execute_medoid(task)  # returns ('done'|'blocked'|'pending', detail)",
        "        if status == 'done':",
        "            completed.append(task)",
        "        elif status == 'blocked':",
        "            blockers.append({'task': task, 'reason': output})",
        "        else:",
        "            pending.append(task)",
        "",
        "    # 8. verify — post-impl smoke / oracle [Exact]; updates completed/pending in-place",
        "    verify_oracles(completed, pending, blockers)  # re-classifies after oracle run",

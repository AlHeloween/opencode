...
export function setupSlots(api: HostPluginApi): HostSlots {
  const reg = createSolidSlotRegistry<RuntimeSlotMap, TuiSlotContext>(
    ...
    {
      onPluginError(event) {
        Log.Default.warn("bug: tui.slot plugin error", {
  ...
  return {
    register: <Slots extends Record<string, object>>(plugin: HostSlotPlugin<Slots>) => {
      if (!isHostSlotPlugin(plugin)) return () => {}
      return reg.register(plugin as HostSlotPlugin<Record<string, object>>)
    },
  }
}

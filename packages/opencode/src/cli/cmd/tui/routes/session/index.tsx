...
import type { ApplyPatchTool } from "@/tool/applypatch"
import type { WebFetchTool } from "@/tool/webfetch"
import type { UniversalSearchTool } from "@/tool/universalsearch"
import type { CapabilityTool } from "@/tool/capability"
import type { PipelineTool } from "@/tool/pipeline"
...
export function Session() {
  ...
  const unsub1 = event.on("message.part.updated", (evt) => {
  ...
  const unsub2 = event.on("session.status", (evt) => {
  ...
  return (
    <Show when={!shouldHide()}>
      <Switch>
        ...
        <Match when={props.part.tool === "pipeline"}>
          <Pipeline {...toolprops} />
        </Match>
      ...
      <Switch>
        ...
        <Match when={true}>
          <text paddingLeft={3} fg={fg()} attributes={denied() ? TextAttributes.STRIKETHROUGH : undefined}>
            <Show fallback={<>{props.spinner ? "…" : "—"} {props.pending}</>} when={props.complete}>
...
function Capability(props: ToolProps<typeof CapabilityTool>) {
  const { theme } = useTheme()
  const output = createMemo(() => props.output?.trim() ?? "")
  const [expanded, setExpanded] = createSignal(false)
  const lines = createMemo(() => output().split("\n"))
  ...
  const task = createMemo(() => props.input.task ?? "Capability lookup")

  return (
    <Switch>
      <Match when={output()}>
        <BlockTool
          title={`# Capability lookup ${task()}`}
          part={props.part}
          onClick={overflow() ? () => setExpanded((prev) => !prev) : undefined}
        >
          <box gap={1}>
      ...
      <Match when={true}>
        <InlineTool icon="⌕" pending="Looking up capability..." complete={task()} part={props.part}>
          Capability {task()}
        </InlineTool>
      </Match>
    </Switch>
...
function Pipeline(props: ToolProps<typeof PipelineTool>) {
  const { theme } = useTheme()
  const output = createMemo(() => props.output?.trim() ?? "")
  const [expanded, setExpanded] = createSignal(false)
  const lines = createMemo(() => output().split("\n"))
  ...
  const steps = createMemo(() => props.input.steps?.length ?? 0)

  return (
    <Switch>
      <Match when={output()}>
        <BlockTool
          title={`# Pipeline (${steps()} steps)`}
          part={props.part}
          onClick={overflow() ? () => setExpanded((prev) => !prev) : undefined}
        >
          <box gap={1}>
      ...
      <Match when={true}>
        <InlineTool icon="⑂" pending={`Pipeline running (${steps()} steps)...`} complete={`Pipeline: ${steps()} steps`} part={props.part}>

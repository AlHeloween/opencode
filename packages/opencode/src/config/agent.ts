...
const AgentSchema = Schema.StructWithRest(
  Schema.Struct({
    ...
    // mutable: Config merge assigns into non-readonly AgentConfig maps
...
export async function load(dir: string) {
  ...
  for (const item of await Glob.scan("{agent,agents}/**/*.md", {
    ...
    const md = await ConfigMarkdown.parse(item).catch(async (err) => {
      ...
      void Bus.publish(Session.Event.Error, { error: new NamedError.Unknown({ message }).toObject() }).catch((e) => log.debug("config agent error publish failed", { error: String(e) }))

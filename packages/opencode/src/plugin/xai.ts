...
// holes тАФ the user opens the URL on any device with a browser, types
...
// `null`, or `-5` from a misbehaving device-code endpoint тАФ without this,
...
export async function pollDeviceCodeToken(
  ...
  while (now() < deadline) {
    ...
    // RFC 8628 ┬з3.5: authorization_pending = keep polling at the same
    // interval; slow_down = bump the interval by тЙе5s and keep polling.
    ...
    auth: {
      ...
      async loader(getAuth) {
        ...
        return {
          ...
          // We intentionally do NOT set baseURL тАФ @ai-sdk/xai already defaults
          ...
          async fetch(requestInput: RequestInfo | URL, init?: RequestInit) {
            ...
            // skew window, or тАФ for JWT access tokens тАФ when the JWT exp
              ...
              if (!refreshPromise) {
                ...
                refreshPromise = refreshAccessToken(refreshToken, options)
                  .then(async (tokens) => {
                    ...
                    // 4xx and force re-login тАФ a known cross-process limitation.

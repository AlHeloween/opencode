...
### Planning Exception: cmd_runner Sessions
For cmd_runner session control/inspection, do not launch general. Manage status, tail, send, stop, and log interpretation directly in the primary planning context. Use delegation only for separate read-only code or log discovery outside the session-control loop.
...
**Guidelines:**
- **Default**: Launch at least 1 general agent for most tasks - it helps validate your understanding and consider alternatives
...
- Include the Explore agent validation result after plan creation; run the Explore agent against the codebase and correct the plan before implementation
...
After implementation, before moving any completed plan to plans_completed/, run the Explore agent against the real code execution state. Move the plan only after the explorer check confirms the code and plan agree.

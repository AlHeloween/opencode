...
import { acquire, listAcquired, releaseAll, releaseItem, tdaHeaderFor } from "@/session/acquired-item-store"
  ...
  it.live("held → withheld when the span passes; released → withheld at once", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const session = yield* Session.Service
        const sid = (yield* session.create({})).id
        // The row is written with RAW SQL on purpose: this test's subject is the turn COUNTER's query, and
        // a fixture built through the message schema's own payload type would tie the assertion to a shape
        // it does not care about — the counter needs a user ROLE and nothing else.
        //
        // It used to insert into `session_entry`, and THAT is why the defect survived: nothing in this build
        // writes that table (0 rows on a live database against 15 368 messages, 1 577 of them user turns),
        // so the counter returned 0 in production while these tests stayed green — the fixture and the code
          ...
          projectDb((db) =>
            db.run(
              sql`INSERT INTO "message" (id, session_id, time_created, time_updated, data)
                  VALUES (${MessageID.make(id)}, ${sid}, ${Date.now()}, ${Date.now()}, '{"role":"user"}')`,
            ),
          )

...
let summaryOrdering:
  | {
      calls: number
      started: ReturnType<typeof defer<void>>
      release: ReturnType<typeof defer<void>>
      persist: () => Effect.Effect<void>
      messageID?: MessageID
    }
  | undefined

const orderedSummary = Layer.succeed(
  SessionSummary.Service,
  SessionSummary.Service.of({
    summarize: (input) =>
      Effect.gen(function* () {
...
function makeHttp(summaryLayer = summary) {
  ...
  const proc = SessionProcessor.layer.pipe(Layer.provide(summaryLayer), Layer.provideMerge(deps))
  ...
  ).pipe(Layer.provide(summaryLayer))
}
...
it.live("native mode transition keeps the provider identity and declared tools stable", () =>
  provideTmpdirServer(
    Effect.fnUntraced(function* ({ llm }) {
      ...
      expect(JSON.stringify(reasoningMessages)).toContain("## 0. WORKFLOW")
    }),
    { git: true, config: providerCfg },
  ),
  30_000,

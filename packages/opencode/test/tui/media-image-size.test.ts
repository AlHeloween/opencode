...
describe("MediaImage native pixel sizing (contain-fit)", () => {
  ...
  test("wide source is width-limited to maxCols cells, and the 512 px side cap wins", () => {
    const size = nativeImagePixelSize({
      srcWidth: 3000,
      srcHeight: 400,
      maxCols: 80,
    ...
    // The cell budget allows 80*18 = 1440 px, but 512 is the per-SIDE ceiling — the owner
    // restated it 2026-09-21: «512 это сторона если вылазит за границу … скейлить если больше
    // чем 512 иначе мы получим громоздкие вставки что совсем не гуд». The cap only ever shrinks:
    // a source under 512 is returned untouched (see the natural-size tests above).

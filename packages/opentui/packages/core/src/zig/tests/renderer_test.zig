...
test "renderer - hides before sixel and restores the input cursor inside one synchronized frame" {
    const pool = gp.initGlobalPool(std.testing.allocator);
    defer gp.deinitGlobalPool();
    var local_link_pool = link.LinkPool.init(std.testing.allocator);
    defer local_link_pool.deinit();
    ...
    const cursor_hide = std.mem.indexOf(u8, output, ansi.ANSI.hideCursor) orelse return error.TestExpectedEqual;
    const cursor_move = std.mem.indexOf(u8, output, "\x1b[4;3H") orelse return error.TestExpectedEqual;
    const cursor_show = std.mem.indexOf(u8, output, ansi.ANSI.showCursor) orelse return error.TestExpectedEqual;
    ...
    try std.testing.expect(cursor_hide < image);
    try std.testing.expect(image < cursor_move);
    try std.testing.expect(cursor_move < cursor_show);

...
2. Choose **Claude Pro/Max (browser)** to open the Anthropic authorization page,
   or **Claude Pro/Max (paste code)** when the browser cannot reach the
   OpenCode machine. The browser flow returns to a local callback automatically;
   the paste-code flow accepts either the final redirect URL or `code#state`.
...
3. Now all the Anthropic models available to your account should be available
   when you use the `/models` command.
...
:::note
Anthropic controls subscription eligibility and OAuth grant lifetime. If a
request reports an expired or revoked OAuth grant, run `/connect` again and
complete a fresh Claude Pro/Max login.
:::

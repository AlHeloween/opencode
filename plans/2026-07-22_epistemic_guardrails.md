...
|---|---------------|--------------|----------------|
| 1 | `bun test test/session/constitution.test.ts` from `packages/opencode` | pass (existing suite) | recorded historically in plan (commits + 78-pass note) |
| 2 | `bun test test/session/compaction.test.ts` from `packages/opencode` | pass for key-decisions / summary paths | same |

### Post-implementation oracles
| # | Command (cwd) | Pass criteria |
|---|---------------|---------------|
| 1 | `bun test test/jobs/jobs.test.ts test/session/constitution.test.ts test/session/compaction.test.ts` from `packages/opencode` | all pass |
| 2 | `bun typecheck` from `packages/opencode` | exit 0 (remaining open item) |

### Gate
- [x] Smoke requirements written
- [x] Baseline recorded [Exact] (prior implementation cycles)
- [x] Implementation only after baseline
- [ ] Post-impl smoke passed before final [x] — typecheck still open

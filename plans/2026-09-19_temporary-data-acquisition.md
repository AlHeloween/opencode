...
| **T6** | the TTL FIELD — the owner's final form: a `ttl` on the part itself, TWO tools, ALL spammy tools | `ttl` on the part schema (`FilePart` / tool state, so it rides the part's own `data` — no new table, no migration) · `TempEnable` / `TempDisable` returning id + report + ids_range · the conversion gate reading the PART's own ttl against ONE number (the current turn) · summary exclusion · fold auto-reset | **DESIGNED — 2026-09-19.** Owner, verbatim: «не только для read, а для всех тулов которые спамят… TempEnable и TempDisable(…report…) return id+report+ids_range; ttl=null (permanent)… поле в базе ttl=[null - permanent, tmp_xxx session, число - ttl turns); в summary не считается; доползли до компакта — ок, сбрасываем автоматом, оставляем сообщение». Off by default. **This SUPERSEDES the table shipped in `74f0ab0d8a`** — `held_media`, `session/held-media.ts`, its test and the `expiredMedia: Map` option are DELETED (zero consumers, so the removal costs nothing), and the conversion needs one number instead of a map, which also removes the DB query that broke 42 cases. **BUILT — 2026-09-19, five commits.** `ttlUntil`/`ttlScope` ride in the part's json and are DERIVED into the `part` COLUMNS by the single write point (`projectors.ts`); the migration `20260919000000_part_ttl_columns` carries two columns, the `(session_id, ttl_until)` index, the message-role expression index and the defensive `DROP TABLE IF EXISTS held_media`. The turn counter moved to `session/turn.ts` because it had read `session_entry`, which holds ZERO rows in this build — so no span could ever have expired while every suite stayed green, the fixtures having written those rows themselves. The gate is `releaseExpiredParts`: per PART, from the part's own field, REPLACING the payload, naming the part id as the address, treating `null` and `undefined` alike as "no declaration", and releasing an errored result the same way. The current turn now reaches EVERY conversion of history — the three request-assembly branches, both checkpoint saves and `captureSummary` — because a checkpoint must store what the trunk sends. The writers are `tempenable` / `tempdisable` (`Tool.define` refuses separators, so the model-facing names are the owner's `TempEnable`/`TempDisable` lowercased), returning `id` + `report` + `ids_range`, over `session/stored-part.ts` as the ONE lookup that reads the identity from the COLUMNS so a write cannot silently no-op. Oracles: `bun typecheck` exit 0 · `temp-lifetime + recall + part-ttl-gate + part-ttl-storage + message-v2` **73 pass / 0 fail / 169 expect** · `prompt.test.ts` 42 pass / 13 skip / 0 fail at the recorded baseline. **The fold auto-reset LANDED — 2026-09-19:** `spansToExpire` + `expiredSpan` in `message-v2.ts` (the SAME predicates the wire gate uses), applied at the fold seam beside `AcquiredSet.releaseAll`; a running span is moved one turn into the past so the release bites in that very turn, and the message keeps its address. **Residual: live proof** (nothing here is in a deployed binary), and **the summary's ordering** — the fold's own summary is captured BEFORE the fold is decided, so a span still live at that instant appears in it as live and becomes a pointer from the next request on; the strict reading of «в summary не считается» needs the seam re-ordered. |
| **T7** | the release report | the RECORDED diffs (fossil leaves, tool `filediff`s) | the report names what the record shows changed, never what is recalled |
| **T8** | acquire a STORED RECORD — a `project_checkpoint` row, a message range — by id | the SQLite plane + the runtime's set | priced by `data.tokens` for an assistant message, by measure otherwise; **refused** when it would cross `usable()`; released to a pointer like any other item |
| **T9** | epoch-addressable memory — pull a specific earlier summary, hold it for the span, release it | T8 + §0.8 | the summary is in the request while held and absent after the release, and the same one is re-acquirable by id |
...
4. `DROP TABLE held_media` — **DEFENSIVE, not a repair.** The live database NEVER had the table: the DDL that would have created it only ever ran inside tmpdir instances, because no runtime carrying it was ever built. Measured 2026-09-19 by the migration oracle, which demanded a starting state that did not exist. Zero consumers.

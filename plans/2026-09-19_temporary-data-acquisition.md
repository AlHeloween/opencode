...
| **T3c** | the SEND SITE: the header in `llm.ts`, inside the `providerID.startsWith("opencode")` block — created, not joined, since `x-opencode-has-attachments` has no writer | `session/llm.ts` | a session with no items sends NO header; with an item the header appears on an `opencode/*` request and NOT on a third-party one; and no request body changes until an item is actually withheld |

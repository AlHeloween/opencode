...
- [ ] **T2 — inventory the in-window units.** Plans touched in-window (`git log`), memory blocks written in-window, shipped commits; one line each. Deliverable: the list §C2 is computed over.
...
None yet — the plan owes all four tasks. S2/S3 of `plans_completed/2026-09-24_grounding-first-and-boundary-reporting.md` remain its own residual (S2 read once: the tool-first share did not move; the fold-boundary window carries no tool demonstration; S3 needs a plan approval→closure under the new prefix).

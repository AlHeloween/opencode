...
- [ ] **T2 — close the TUI gateway-row plan (BLOCKED on instrument, 2026-09-24).** Isolated candidate ran (version 10.0.1117, `dist/bin/opencode.exe`, cwd `experiments/2026-09-24_tui-protocol-smoke/wc/`, config `bin/` via `OPENCODE_TEST_CONFIG`; live `bin/` untouched, four runs, screenshots kept). The gateway fact was published and matches the expectation (`using:"h3"`, then `using:"h2"` on fallback — candidate log), but the sidebar ROW could not be captured: `cmd_runner screenshot` cuts the window's right edge at 110/160/200 cols, and HF has no credits for a successful turn. The plan stays on the shelf; the attempt is recorded in its A2 row and in `_progress_log`. Next instrument: a capture path that keeps the full window (cua window capture, or the owner's own sidebar) plus a provider with credits.

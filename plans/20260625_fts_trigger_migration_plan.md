...
- [ ] 1. Append FTS5 DDL to `migration/20260601000000_baseline_local_development/migration.sql`
- [ ] 2. Add FTS5 entries to `migration/20260601000000_baseline_local_development/snapshot.json`
- [ ] 3. Remove or guard `FTS_SCHEMA_SQL` execution in `db.ts` (keep as safety net with `IF NOT EXISTS` or remove entirely)
- [ ] 4. Add test: verify FTS5 triggers fire after clean migration (insert part → text appears in FTS)
- [ ] 5. Add test: verify part_fts survives WAL checkpoint + VACUUM (virtual table rebuild)

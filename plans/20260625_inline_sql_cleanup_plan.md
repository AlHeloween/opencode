...
- [ ] 1. Migrate `embedding.ts:47-49` (SELECT existing embedding) to Drizzle query builder
- [ ] 2. Migrate `embedding.ts:70-73` (INSERT embedding) to Drizzle query builder
- [ ] 3. Migrate `embedding.ts:97-106` (dynamic query with conditional WHERE) to Drizzle query builder
- [ ] 4. Migrate `migration.ts:45` (SELECT id FROM migration) to Drizzle query builder
- [ ] 5. Migrate `migration.ts:68,75` (INSERT INTO migration) to Drizzle query builder
- [ ] 6. Add `@sql-intentional` comment markers on all raw SQL that must stay raw
- [ ] 7. Add test: embedding CRUD via Drizzle matches raw SQL behavior
- [ ] 8. Add test: migration tracking via Drizzle works correctly
- [ ] 9. Run typecheck

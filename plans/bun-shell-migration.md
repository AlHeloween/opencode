...
**Status (audited 2025-05-14):** The original Bun Shell `$` template literal has been fully eliminated from `packages/opencode/src`. The baseline numbers in the plan (143 `$` calls across 17 files) are STALE — zero `$` calls remain. What remains is consolidating the various ad-hoc process execution patterns into a unified API.

**What's done:**
- `Process.run()`, `Process.text()`, `Process.lines()` exist in `util/process.ts` (23 call sites across 10 files)
- Git operations migrated to Effect-based `Git.Service` (`src/git/index.ts`) using `ChildProcessSpawner`
- All Bun Shell `$` template literals have been replaced
- `Process.status()`, `Process.shell()`, `Process.git()`, `Process.gitText()` not yet implemented

**Remaining work:** Unify `Process.*`, `Git.Service`, and `ChildProcessSpawner` usage patterns across the codebase. Remove dead wrappers and one-off patterns.

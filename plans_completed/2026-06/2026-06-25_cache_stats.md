...
- [x] 1.1 Add `cache_stats` table to `src/session/session.sql.ts` with Drizzle schema
- [x] 1.2 Columns: `id` (PK, compound), `session_id`, `model_id`, `turn`, `timestamp`, `data` (JSON text)
- [x] 1.3 Unique constraint: `(session_id, model_id, turn)`
- [x] 1.4 Index: `(session_id, model_id, turn)` for fast last-N-row lookup

# TUI чёрный экран на повторном старте — ротация логов блокировала воркер до `Rpc.listen`

**Status:** COMPLETE (G9) — класс дефекта устранён и доказан A/B-оракулом на shipped-бинарнике; остаточные гипотезы закрыты 2026-09-12
...
**ЗАКРЫТО 2026-09-12.** Первоначальная остаточная гипотеза («воркер не отвечает на RPC `fetch`, пока проект обслуживает живая сессия; PID 20148 держит БД») — **не подтвердилась** при изоляции:

| Проверка | Условия | Результат [Exact] |
|---|---|---|
| Deployed-артефакт | `bin\opencode.exe` (PATH) vs `dist\bin\opencode.exe` | побайтово идентичны, оба **10.0.976**, SHA256 `12D46C4E…E468302942` — stale-версии нет |
| Causal A/B, shipped binary | изолированный worktree, **150** логов (> `keep=100`) | ✅ `instance boot completed` → `tui plugins ready` → UI за ~4 с |
| Causal A/B, pre-fix binary | та же площадка, 10.0.957 (`bin_tst\2026-09-09-Stable`) | ❌ обрыв на `kv state absent`, только `dedup flush`; ни `instance boot completed`, ни `tui plugins ready` — **чёрный экран** спустя 50+ с |
| Конкурентный запуск | два инстанса 10.0.976 в одном worktree, общая БД | ✅ **оба** дошли до `tui plugins ready` — гипотеза «живая сессия блокирует второй старт» **опровергнута** |

Исходный in-project `ERROR tui bootstrap failed { "RPC call timed out after 30000ms: fetch" }` объясняется **той же** блокирующей ротацией логов (>100 файлов в каталоге проекта) и снят фиксом.

Остаётся Unknown (не воспроизводится в изоляции): влияет ли существенно бо́льшая реальная БД проекта (152 МБ) на время старта. Действий не требуется.
...
## Open question — ЗАКРЫТ

Исходный вопрос периода `plan_mode` («нужен запуск из бинарника с удержанным локом; для этого требуется build_mode») снят: причина установлена и доказана A/B на shipped-бинарнике (см. Root cause и Residual выше).

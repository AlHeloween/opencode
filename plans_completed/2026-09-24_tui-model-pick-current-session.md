...
Статус: ЗАКРЫТ (2026-09-24, перенесён в `plans_completed/`). Ветка: `Local_Development`.
...
- [x] T5 — владелец подтвердил живой выбор модели после перезапуска TUI (2026-09-24, readme полки: «выбор модели TUI … протестирован вживую»); машинный read-back (строка модели / session settings) в worktree не сохранился — история сессий 09-23/24 отсутствует. Отложенная `refreshSessionSettings()` тем же sid остаётся отдельным кандидатом (фальсификатор: «задержать загрузку, выбрать Sol, завершить старую загрузку и прочитать активную модель»): наблюдением владельца не воспроизведён и не исключён.

Риски: установленный бинарник проверен по хешу и версии; повторный выбор в живой TUI-сессии — ОБНОВЛЕНО 2026-09-24: владелец подтвердил живой выбор (readme полки), C1 закрыт на этом подтверждении (машинного read-back нет). Отложенная `refreshSessionSettings()` для того же sid потенциально может восстановить старую модель после выбора, если чтение началось раньше записи; это отдельный кандидат с фальсификатором «задержать загрузку, выбрать Sol, завершить старую загрузку и прочитать активную модель». До воспроизведения — Inferred риск, не Exact причина этого случая.

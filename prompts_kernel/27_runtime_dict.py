...
RUNTIME_RULES = MappingProxyType({
    ...
    "NAMING": "rule identifiers use UPPER_SNAKE_CASE with underscore '_' delimiter (PEP 8 convention). Dots '.' and hyphens '-' are forbidden in new rule names. Legacy dotted rules (EVIDENCE_ORDER, SEARCH_ORDER, REUSE_BEFORE, INFOMARK_SEP, WRITE_SCOPE, CACHE_STABILITY, VERIFY_OUTCOME, ADID_FREEZE, ADID_OPS, NO_HARDCODE, WHERE_WHICH, SV_OUTPUT, CLEAN_STATE, DECOMPOSE, DOCUMENT_SURFACE, WORKSPACE_LANES, PROGRESS_LOG, MEMORY_RANK, MEMORY_LINKS) are grandfathered — do not extend the dotted namespace with new entries.",
    ...
    "VCS_ROOT": "VCS metadata directory (.git/, .fossil/, .hg/) must be at repo root only. Never search inside, read from, or interact with _FOSSIL_ files, .git/objects, .hg/store, or any VCS internals as if they were project content. VCS metadata is NOT source code — treat as opaque infrastructure. If you see a _FOSSIL_ file outside repo root, report it; do NOT cd into it or grep it.",

...
RUNTIME_RULES = MappingProxyType({
    ...
    "EMIT_STATE": "emit_state(goal_sv, completed_tasks, pending_tasks, blockers, next_step, out_of_scope, terminal) returns a structured state dict {done, pending, blocked, next, goal_sv, out_of_scope, terminal}. v6: +out_of_scope (discarded tasks), +terminal (bool — true when pending=[] and blockers=[]). Serialised by the caller; InfoMark-stamped at Gate 9.",

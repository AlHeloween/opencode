...
def render_algorithm_card() -> str:
    ...
    lines = [
        ...
        "",
        "# === STEP 9: emit_state + residual — ADID loop closure (v6) ===",
        "state = emit_state(",
        "    goal_sv=goal_sv,",
        ...
        "    blockers=blockers,",
        "    next_step=next_task,",
        "    out_of_scope=discarded,   # v6: tasks that didn't pass Goal-SV threshold",
        "    terminal=(len(pending) == 0),  # v6: empty residual → TERMINAL",
        ")",
        "residual = residual_recluster(state, original_goal_sv=goal_sv)",
        "# residual may be [] → TERMINAL (v6). Discarded tasks → out_of_scope.",
        ...
        "# follows the full sequence. Speed comes from evidence density, not task size.",

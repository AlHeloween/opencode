...
from codex_svchain import format_chain, select_session, vector_in, vectors
...
def test_order_filter_and_broken_edges(tmp_path: Path) -> None:
    ...
    transcript = tmp_path / "rollout-example.jsonl"
...
def test_reject_incomplete_block_and_ambiguous_selector() -> None:
    ...
    files = [Path("rollout-one.jsonl"), Path("rollout-two.jsonl")]
    try:
        select_session(files, "rollout")
